"""
recorder.py – Session recorder for Bugula.
...
# ---------------------------------------------------------------------------
# JS that gets injected into every page the recorder watches.
# It pushes structured event objects onto window.__bugula_events__.
# ---------------------------------------------------------------------------
_RECORDER_JS = r"""
(function () {
...
# ---------------------------------------------------------------------------
# Selector → CSS path cleaner
# ---------------------------------------------------------------------------
def _clean_selector(sel: str) -> str:
    """Remove characters illegal in YAML plain scalars."""
    return sel.strip()


# ---------------------------------------------------------------------------
# Convert raw JS events → YAML-ready dicts
# ---------------------------------------------------------------------------

# Pauses longer than this (ms) become explicit wait steps in the YAML.
_MIN_WAIT_MS = 1500   # ignore short natural pauses
_MAX_WAIT_MS = 8000   # cap very long manual pauses (coffee break, etc.)


def _events_to_steps(events: list[dict], source_url: str) -> list[dict]:
    steps: list[dict] = []
    last_url = source_url
    last_ts: int | None = None  # JS timestamp (ms) of previous processed event

    # URL at the time the previous step was emitted – used to decide
    # whether source_url is needed (only on URL transitions).
    step_url = source_url
    ...
    while i < len(events):
        ...
        ev_ts   = ev.get("ts")  # ms timestamp set by JS pushEvent()

        # Insert a wait step whenever the user paused noticeably
        if last_ts is not None and ev_ts is not None:
            ...
            if gap_ms > _MIN_WAIT_MS:
                ...
                steps.append({
                    ...
                    "url": "",
                    "source_url": "",
                    "description": f"Wait {wait_s}s",
                    "selector": "",
                    "element_text": "",
                    "input_value": str(wait_s),
                    "submit_key": "",
                })
        last_ts = ev_ts
        ...
        # source_url is only needed when the step happens on a different page
        # than the previous step – avoids redundant driver.get() in the tester.
        nav_source = ev_url if ev_url != step_url else ""
        if ev_url:
            step_url = ev_url

        if ev_type == "input":
            # Peek: is the next event a key (enter/tab)?
            submit_key = ""
            if i + 1 < len(events) and events[i + 1].get("type") == "key":
                submit_key = events[i + 1].get("key", "")
                i += 1  # consume key event too
            value = ev.get("value", "")
            if value == "***":
                value = ""  # don't record passwords
            steps.append({
                "method": "form_input",
                "url": "",
                "source_url": nav_source,
        ...
        if ev_type == "click":
            ...
            # anchor with href → treat as link
        ...
        # key without preceding input (e.g. escape)
        if ev_type == "key":
            i += 1
            continue
...
class SessionRecorder:
    ...
    Usage:
        rec = SessionRecorder(driver, start_url)
        rec.start()
        # user does things in the browser
        rec.stop()
        yaml_text = rec.to_yaml(name="my_recording", url=start_url,
                                browser="chrome")
    """

    POLL_INTERVAL = 0.8   # seconds between JS event polls
    ...
    def _poll_once(self):
        try:
            ...
            if current_url != self._last_url:
                ...
                # re-inject on every page change
                self._inject()

            raw = self.driver.execute_script(_POLL_JS) or []
            self._events.extend(raw)
        except Exception:
            pass   # browser might be mid-navigation
    ...
    # ------------------------------------------------------------------
    def to_yaml(self, name: str = "recording",
                url: str = "",
                browser: str = "chrome",
                username: str = "",
                password: str = "") -> str:
        """Convert captured events to a YAML testcase string."""
        steps = _events_to_steps(self._events, url or self.start_url)
...
# ---------------------------------------------------------------------------
# Deduplicate / clean up raw steps
# ---------------------------------------------------------------------------
def _deduplicate(steps: list[dict]) -> list[dict]:
    """Remove trivially duplicate consecutive steps."""
    out: list[dict] = []
    for s in steps:
        if out and out[-1] == s:
            continue
        # collapse back-to-back link steps to same URL
