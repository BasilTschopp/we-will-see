...
- Matrix runs: run the same test case with multiple variable combinations, each producing its own result set.
- Parallel execution: run multiple selected test cases concurrently, each in its own browser session.
- Stored values: capture a value from the page and reference it as a variable in later steps.
- Optional steps: mark a click step as optional so a missing element skips it instead of failing the run.
- Presence/absence assertions: verify that an element does or does not appear on the page after an action.
- Execution settings: configure step timeout, run timeout, stop-on-error, parallel execution, and screenshot-on-error, applied to every test run.
...
- `We Will See.bat` automatically creates a venv, installs dependencies, and launches the application if needed.
